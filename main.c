...
char scroller_message[] = 
	...
	"Use D-Pad to move Alex, Circle to jump and Cross to shoot      "
	"Code and GFX by Johan Peitz      Music and SFX by Anders Svensson       PSP port by MK2k";
...
void show_cutscene(int level) {
	...
	while(mode != 3) {
		...
		while(cycle_count > 0) {
			...
			//if (mode == 1 && (keypressed() || is_fire(&ctrl)) || my_counter > 200) {
				mode = 2;
			}
...
int do_pause_menu(BITMAP *bg) {
	...
	textout_outline_center(bg, "triangle to quit", 80, 60);
...
int do_main_menu() {
	...
	if (status == GS_CUSTOM) {	// User selected Custom
		...
		{
			//start_music(MOD_INTRO_SONG); // by MK2k
			stop_music();
